...
module.exports = {
  ...
  getResource({
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!url) {
        if (!partition) {
          partition = config.getConf('mCSD:registryDB');
        }
        url = URI(config.getConf('mCSD:url')).segment(partition).segment(resource);
  ...
  saveResource({
    ...
    const url = URI(config.getConf('mCSD:url')).segment(partition).toString();
    const options = {
      url,
      headers: {
