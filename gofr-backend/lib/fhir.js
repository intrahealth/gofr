...
module.exports = function () {
  return {
    sync(host, username, password, mode, name, sourceOwner, clientId, topOrgName) {
      ...
      this.getLastUpdate(database, (lastUpdated) => {
        let url;
        const baseURL = URI(host).segment('Location').segment('_history').toString();
        if (mode === 'update') {
        ...
        async.doWhilst(
          ...
          () => {
            ...
            const url = URI(config.get('mCSD:url')).segment(database)
              .segment('fhir')
              .segment('Location')
              .segment(fhir.id)
              .toString();
