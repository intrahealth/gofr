...
module.exports = function () {
  return {
    ...
    restoreDB(archive, db, callback) {
      ...
      this.archiveDB(db, (err) => {
        ...
        this.deleteDB(db, (err) => {
          ...
          const dbList = [];
          dbList.push({
            archive: `${conficonfig.get('mapping:dbPrefix')}${db}_${archive}.tar`,
            db: `${config.get('mapping:dbPrefix')}${db}`,
          });
          dbList.push({
    ...
    archiveDB(db, callback) {
      ...
      dbList.push({
        name: `${config.get('mapping:dbPrefix')}_${name}`,
        db: `${config.get('mapping:dbPrefix')}${db}`,
      });
      const error = false;
      async.eachSeries(dbList, (list, nxtList) => {
    ...
    getArchives(db, callback) {
      const filter = function (stat, path) {
        if (path.includes(db) && !path.includes(config.get('mapping:dbPrefix'))) {
          return true;
        }
        return false;
      };
    ...
    cleanArchives(db, callback) {
      const maxArchives = config.get('dbArchives:maxArchives');
      const filter = function (stat, path) {
        if (path.includes(db) && !path.includes(config.get('mapping:dbPrefix'))) {
          return true;
        }
        ...
        if (files.length > maxArchives) {
          ...
          async.eachSeries(files, (file, nxtFile) => {
            ...
            filesDelete.forEach((fileDelete) => {
              ...
              fileDelete = `${dl[0]}${config.get('mapping:dbPrefix')}_${db}${dl[1]}`;
              fs.unlink(fileDelete, (err) => {
                if (err) {
                  logger.error(err);
                }
