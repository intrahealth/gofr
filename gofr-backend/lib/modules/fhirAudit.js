...
const fhirAudit = {
  ...
  save: (audit) => {
    fhirAxios.create(audit, 'DEFAULT').catch((err) => {
  ...
  audit: () => {
    const audit = JSON.parse(JSON.stringify(AUDIT_TEMPLATE));
    const now = new Date();
    audit.recorded = now.toISOString();
    if (config.get('audit:observer')) {
      delete audit.source.observer.identifier;
      audit.source.observer.reference = config.get('audit:observer');
    }
  ...
  startup: () => {
    const startupAudit = fhirAudit.audit();
    startupAudit.type = {
  ...
  login: (user, ip, success, email) => {
    const loginAudit = fhirAudit.audit();
    loginAudit.type = {
      system: 'http://dicom.nema.org/resources/ontology/DCM',
      code: '110100',
    };
  ...
  logout: (user, ip) => {
    const logoutAudit = fhirAudit.audit();
    logoutAudit.type = {
      system: 'http://dicom.nema.org/resources/ontology/DCM',
      code: '110100',
    };
  ...
  create: (user, ip, what, success, detail) => {
    const createAudit = fhirAudit.audit();
  ...
  update: (user, ip, what, success, detail) => {
    const updateAudit = fhirAudit.audit();
